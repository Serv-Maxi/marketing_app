...
import CreateCompanyDialog from "./_components/company-create-dialog";
import { Input } from "@/components/ui/input";
import CompanyRowActions from "./_components/row-actions";
...
export default function CompaniesPage() {
  ...
  const refetch = React.useCallback(async () => {
    setLoading(true);
    try {
      const list = await adminCompaniesService.list();
      setCompanies(list);
    } catch {
      /* silent */
    } finally {
  ...
  const columns: ColumnDef<CompanyRow>[] = [
    ...
    { key: "name", header: "Name" },
    {
      key: "actions",
      header: "",
      cell: (r) => <CompanyRowActions company={r} onChanged={refetch} />,
